...
export function ChronicleStoryWorkspace({ chronicleId, associatedCharacters }: Props) {
    ...
    <section className="story-workspace" aria-label="Historias de la crónica">
      ...
      <main className="story-detail">
        {selected === null ? <div className="story-detail__empty"><span>♜</span><h2>Crea la primera historia</h2><p>Aquí aparecerán sus hitos, sesiones, reparto y localizaciones.</p></div> : <>
          ...
          <section className="story-section"><div className="story-section__heading"><div><span>Desarrollo</span><h3>Sesiones vinculadas</h3></div><button type="button" onClick={() => setShowContext(true)} disabled={readOnly}>＋ Vincular sesión</button></div><div className="story-session-list">{selected.sessions.map((session) => <article key={session.id}><div className="story-resource-icon">◫</div><div><strong>{session.sessionNumber === null ? 'Sesión' : `Sesión ${session.sessionNumber}`}: {session.title ?? 'Sin título'}</strong><small>{storyDate(session.realDate)} · {session.status}</small><textarea value={sessionNotes[session.id] ?? ''} disabled={readOnly} onChange={(event) => setSessionNotes((current) => ({ ...current, [session.id]: event.target.value }))} placeholder="Qué avanzó en esta historia…" /><button type="button" disabled={readOnly || operation !== null} onClick={() => void perform(`session:${session.id}`, () => storyGateway.updateSessionProgress(chronicleId, selected.id, session.id, { expectedRevision: selected.revision, progressNotes: sessionNotes[session.id]?.trim() || null }))}>Guardar avance</button></div></article>)}{selected.sessions.length === 0 ? <button type="button" className="story-add-card" onClick={() => setShowContext(true)}>＋ Añadir sesión relacionada</button> : null}</div></section>

          <section className="story-section"><div className="story-section__heading"><div><span>Cronología</span><h3>Sucesos registrados</h3></div><button type="button" onClick={() => setShowContext(true)} disabled={readOnly}>＋ Añadir suceso</button></div><div className="story-compact-grid">{selected.events.map((event) => <article key={event.id}><span className="story-resource-icon">◇</span><div><strong>{event.title}</strong><small>{event.narrativeTimeLabel ?? storyDate(event.realDate)}</small></div></article>)}{selected.events.length === 0 ? <button type="button" className="story-add-card" onClick={() => setShowContext(true)}>＋ Relacionar suceso</button> : null}</div></section>

          <section className="story-section story-cast"><div className="story-section__heading"><div><span>Reparto</span><h3>Personajes implicados</h3></div><button type="button" onClick={() => setShowContext(true)} disabled={readOnly}>＋ Gestionar reparto</button></div><div className="story-cast__columns"><div><h4>Personajes jugadores</h4>{selected.characters.map((link) => { const character = associatedCharacters.find((item) => item.characterId === link.id); return <article key={link.id}><span className="story-avatar">PJ</span><div><strong>{character?.name || 'Personaje sin nombre'}</strong><small>{character?.concept ?? 'Sin concepto'}</small></div></article> })}{selected.characters.length === 0 ? <p className="story-empty">Ningún PJ implicado.</p> : null}</div><div><h4>PNJ</h4>{selected.npcs.map((npc) => <article key={npc.id}><span className="story-avatar story-avatar--npc">PNJ</span><div><strong>{npc.name}</strong><small>{npc.narrativeRole ?? npc.category ?? 'Sin rol definido'}</small></div></article>)}{selected.npcs.length === 0 ? <p className="story-empty">Ningún PNJ implicado.</p> : null}</div></div></section>

          <section className="story-section"><div className="story-section__heading"><div><span>Escenarios</span><h3>Localizaciones</h3></div><button type="button" onClick={() => setShowContext(true)} disabled={readOnly}>＋ Añadir localización</button></div><div className="story-compact-grid">{selected.locations.map((location) => <article key={location.id}><span className="story-resource-icon">⌂</span><div><strong>{location.name}</strong><small>{location.category ?? 'Localización'}</small></div></article>)}{selected.locations.length === 0 ? <button type="button" className="story-add-card" onClick={() => setShowContext(true)}>＋ Relacionar localización</button> : null}</div></section>
        </>}
      </main>
      ...
      <aside className="story-sidebar">
        {selected === null ? <div className="story-sidebar__placeholder">Selecciona una historia para ver su resumen.</div> : <>
          ...
          <section className="story-side-card story-closure"><div className="story-side-card__heading"><span>♜</span><h3>Cierre de historia</h3></div><p>{selected.status === 'completed' ? `Historia cerrada: ${selected.closure.completion?.grantedCount ?? 0} concesiones de +1 XP registradas.` : selected.closure.eligibleCharacterCount === 0 ? 'No hay personajes elegibles. Puedes cerrar, pero no se concederá experiencia.' : 'El cierre otorgará +1 XP según la asistencia real a las sesiones de esta historia.'}</p><ul><li className={selected.progress.completed === 5 ? 'is-ready' : ''}>{selected.progress.completed}/5 hitos completados</li><li className={selected.closure.hasEligibleSession && !selected.closure.hasPreparationSession ? 'is-ready' : ''}>Sesiones finalizadas, sin sesiones en preparación</li><li className={selected.closure.hasEligibleSession ? 'is-ready' : ''}>{selected.closure.eligibleCharacterCount} personajes elegibles por asistencia</li></ul>{selected.status !== 'completed' ? <><label className="story-closure__resolution"><span>Resolución narrativa</span><textarea value={resolutionDraft} disabled={readOnly} onChange={(event) => setResolutionDraft(event.target.value)} placeholder="Cómo termina esta historia…" maxLength={8000} /></label><label className={`story-closure__confirm${selected.closure.eligibleCharacterCount === 0 ? ' is-warning' : ''}`}><input type="checkbox" checked={completionConfirmed} disabled={readOnly} onChange={(event) => setCompletionConfirmed(event.target.checked)} /><span>{selected.closure.eligibleCharacterCount === 0 ? 'Confirmo el cierre sin conceder experiencia' : `Confirmo el cierre y la concesión de +1 XP a ${selected.closure.eligibleCharacterCount} personaje${selected.closure.eligibleCharacterCount === 1 ? '' : 's'}`}</span></label></> : null}<button type="button" className="story-closure__button" disabled={!closureEligible || operation !== null} onClick={() => void perform('complete', () => storyGateway.complete(chronicleId, selected.id, { expectedRevision: selected.revision, operationId: completionOperationId, resolution: resolutionDraft.trim(), confirmed: true }))}>{selected.status === 'completed' ? '✓ Historia cerrada · +1 XP' : operation === 'complete' ? 'Cerrando historia…' : 'Cerrar historia · +1 XP'}</button>{!readOnly ? <button type="button" className="story-archive" onClick={() => void perform('archive', () => storyGateway.archive(chronicleId, selected.id, selected.revision))}>Archivar historia</button> : null}</section>
        </>}
      </aside>
      ...
      {showContext && selected !== null ? <div className="story-modal" role="dialog" aria-modal="true" aria-labelledby="story-context-title"><div className="story-modal__panel"><header><div><span>Relaciones de la historia</span><h3 id="story-context-title">Vincular contenido</h3></div><button type="button" onClick={() => setShowContext(false)} aria-label="Cerrar">×</button></header><div className="story-modal__grid">
        <ContextGroup title="Sesiones" items={sessions.map((item) => ({ id: item.id, label: `${item.sessionNumber === null ? 'Sesión' : `Sesión ${item.sessionNumber}`} · ${item.title ?? 'Sin título'}` }))} selected={context.sessionIds} onToggle={(id) => setContext((current) => ({ ...current, sessionIds: toggleId(current.sessionIds, id) }))} />
        <ContextGroup title="Sucesos" items={events.map((item) => ({ id: item.id, label: item.title }))} selected={context.eventIds} onToggle={(id) => setContext((current) => ({ ...current, eventIds: toggleId(current.eventIds, id) }))} />
        <ContextGroup title="Personajes jugadores" items={associatedCharacters.map((item) => ({ id: item.characterId, label: item.name || 'Personaje sin nombre' }))} selected={context.characterIds} onToggle={(id) => setContext((current) => ({ ...current, characterIds: toggleId(current.characterIds, id) }))} />
        <ContextGroup title="PNJ" items={npcs.map((item) => ({ id: item.id, label: item.name }))} selected={context.npcIds} onToggle={(id) => setContext((current) => ({ ...current, npcIds: toggleId(current.npcIds, id) }))} />
        <ContextGroup title="Localizaciones" items={locations.map((item) => ({ id: item.id, label: item.name }))} selected={context.locationIds} onToggle={(id) => setContext((current) => ({ ...current, locationIds: toggleId(current.locationIds, id) }))} />
