...
export function NotebookWorkspace() {
  ...
  return <main className="nb2-workspace">
        <header className="nb2-header nb2-header--fixed" style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) minmax(460px,650px)',alignItems:'center',gap:24}}><div className="nb2-header__copy"><small>CRÓNICAS · CUADERNO</small><h1 style={{whiteSpace:'normal',overflow:'visible',wordBreak:'normal'}}>Cuaderno de la crónica</h1><p>Notas. Ideas. Conexiones. Todo deja huella.</p></div><div className="nb2-header__controls"><label>Crónica activa<select aria-label="Crónica activa" value={chronicleId} disabled={loading || composer} onChange={(event) => { setChronicleId(event.target.value); setTag(''); setSearch(''); setSessionFilter(''); setSelectedId('') }}>{chronicles.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button className="nb2-primary" disabled={!chronicleId || loading} onClick={() => startNote()}>✎ Nueva nota</button><button disabled={loading} onClick={() => setRevision((value) => value + 1)}>Actualizar</button></div><div className="nb2-header__cover" aria-hidden="true"><img src={'/api/chronicles/' + chronicleId + '/cover?revision=' + revision} alt="" onError={(event) => { event.currentTarget.style.display = 'none' }} /><span>{chronicles.find((item) => item.id === chronicleId)?.name || 'BloodKeeper'}</span><em>SIEMPRE VIGILA</em></div></header>
