import { rememberNotebookChronicle } from '../../../components/layout/activeChronicleHeader'
...
import './notebook-physical-v1.css'
import { NotebookPhysicalBook } from './NotebookPhysicalBook'
import { NotebookPageTransition } from './NotebookPageTransition'
...
const notebookSectionOrder: readonly Section[] = ['SUMMARY', 'NOTES', 'SESSION', 'TAGS', ...resourceSections]
...
export function NotebookWorkspace() {
  ...
  return <main className="journal-v4 nb2-workspace">
    ...
    <NotebookPhysicalBook>
    {error && <p role="alert" className="nb2-error">{error}</p>}{warning && <p role="status" className="nb2-warning">{warning}</p>}
    {loading ? <p role="status">Cargando cuaderno…</p> : !chronicleId ? <p>No participas en ninguna crónica.</p> : <NotebookPageTransition activeKey={section} order={notebookSectionOrder}> <>
      ...
      {section === 'NOTES' && <section className="nb2-notes">
        <nav className="nb2-filters" aria-label="Filtros de notas">{([['ALL', 'Todo'], ['MINE', 'Mis notas'], ['SHARED', 'Compartido'], ['PINNED', 'Fijado']] as const).map(([key, label]) => <button key={key} aria-pressed={filter === key} onClick={() => setFilter(key)}>{label}</button>)}<label className="nb2-search">Buscar notas<input type="search" value={search} onChange={(event) => setSearch(event.target.value)} /></label><label>Etiqueta<select value={tag} onChange={(event) => setTag(event.target.value)}><option value="">Todas</option>{tagEntries.map(([key, item]) => <option value={key} key={key}>#{item.label} ({item.count})</option>)}</select></label></nav>
