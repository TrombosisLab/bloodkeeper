...
import { NotebookSessionTimeline } from './NotebookSessionTimeline'
...
// NOTEBOOK_COMPOSITION_REBUILD_V1
// NOTEBOOK_CHRONICLE_COVER_HEADER_V1
// NOTEBOOK_MOBILE_READER_NAVIGATION_V1
// NOTEBOOK_SESSIONS_LIST_V1
// NOTEBOOK_VISUAL_POLISH_COMPLETE_V1
export function NotebookWorkspace() {
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([notebookApi.list(chronicleId), notebookApi.context(chronicleId), loadSessions(chronicleId)]).then(([noteResult, contextResult, sessionResult]) => {
      ...
      if (noteResult.status === 'fulfilled') {
        ...
        setCanManage(noteResult.value.canManage)
  ...
  const contextLocationImage = useMemo(() => {
    if (!activeNote || !context) return null
    const locations = new Map((context.locations ?? []).map((item) => [item.id, item]))
    const candidates = new Set<string>()
    for (const ref of activeNote.references) {
      if (ref.targetType === 'LOCATION' && locations.has(ref.targetId)) candidates.add(ref.targetId)
      const linked = context.resources.find((item) => item.id === ref.targetId)?.locationId
      if (linked && locations.has(linked)) candidates.add(linked)
    }
    let best: typeof context.locations[number] | null = null
    let bestDepth = -1
    for (const id of candidates) {
      let current = locations.get(id)
      let depth = 0
      const seen = new Set<string>()
      while (current?.parentLocationId && !seen.has(current.id)) {
        seen.add(current.id); depth += 1; current = locations.get(current.parentLocationId)
      }
      const original = locations.get(id)
      if (original && depth > bestDepth) { best = original; bestDepth = depth }
    }
    return best?.imageUrl ? { ...best, imageUrl: best.imageUrl } : null
  }, [activeNote, context])
  ...
  async function openResource(ref: Mention, full = false) {
    ...
    setPreview(null); setPreviewError(''); setFullSheet(full); setPreviewOpen(full)
  ...
  return <main className="nb2-workspace">
    <header className="nb2-header"><div className="nb2-header__copy"><small>CRÓNICAS · CUADERNO</small><h1>Cuaderno de la crónica</h1><p>Notas. Ideas. Conexiones. Todo deja huella.</p></div><div className="nb2-header__controls"><label>Crónica activa<select aria-label="Crónica activa" value={chronicleId} disabled={loading || composer} onChange={(event) => { setChronicleId(event.target.value); setTag(''); setSearch(''); setSessionFilter(''); setSelectedId('') }}>{chronicles.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button className="nb2-primary" disabled={!chronicleId || loading} onClick={() => startNote()}>＋ Nueva nota</button><button className="nb2-header__refresh" type="button" aria-label="Actualizar cuaderno" disabled={loading} onClick={() => setRevision((value) => value + 1)}>↻</button></div><div className="nb2-header__cover" aria-hidden="true">{chronicleId ? <img src={'/api/chronicles/' + chronicleId + '/cover'} alt="" onError={(event) => { event.currentTarget.style.display = 'none' }} /> : null}<span>{chronicles.find((item) => item.id === chronicleId)?.name || 'BloodKeeper'}</span><em>SIEMPRE VIGILA</em></div></header>
    ...
    {loading ? <p role="status">Cargando cuaderno…</p> : !chronicleId ? <p>No participas en ninguna crónica.</p> : <>
      ...
      {resourceSections.includes(section as ResourceType) && <section className="nb2-panel nb2-resource-panel"><div className="nb2-panel-heading"><h2>{labels[section]}</h2><label>Buscar recurso<input type="search" value={search} onChange={(event) => setSearch(event.target.value)} /></label></div><div className="nb2-catalog">{cards.filter((item) => item.targetType === section && (item.label + ' ' + item.description).toLocaleLowerCase('es').includes(search.toLocaleLowerCase('es'))).map((item) => <article key={item.targetId} className="nb2-resource-card"><img className="nb2-resource-card__image" src={'/api/chronicles/' + chronicleId + '/assets/' + (item.targetType === 'NPC' || item.targetType === 'LOCATION' ? item.targetType : 'RESOURCE') + '/' + item.targetId + '/image'} alt={'Imagen de ' + item.label} loading="lazy" onError={(event) => { event.currentTarget.hidden = true }} /><small>{item.category || labels[section]}{item.restricted ? ' · Solo narrador' : ''}</small><h3>{item.label}</h3><p>{item.description || 'Sin descripción.'}</p><span>{notes.filter((note) => note.references.some((ref) => ref.targetId === item.targetId && ref.targetType === item.targetType)).length} anotaciones accesibles</span><footer><button onClick={() => void openResource(item, true)}>Abrir ficha completa</button><button onClick={() => startNote(item)}>Añadir nota</button></footer></article>)}</div>{!cards.some((item) => item.targetType === section) && <p>No hay {labels[section]?.toLowerCase()} disponibles para tu cuenta en esta crónica.</p>}</section>}
      {section === 'NOTES' && <section className="nb2-notes">
        <nav className="nb2-filters" aria-label="Filtros de notas">{([['ALL', 'Todo'], ['MINE', 'Mis notas'], ['SHARED', 'Compartido'], ['PINNED', 'Fijado']] as const).map(([key, label]) => <button key={key} aria-pressed={filter === key} onClick={() => setFilter(key)}>{label}</button>)}<label className="nb2-search">Buscar notas<input type="search" value={search} onChange={(event) => setSearch(event.target.value)} /></label><label>Etiqueta<select value={tag} onChange={(event) => setTag(event.target.value)}><option value="">Todas</option>{tagEntries.map(([key, item]) => <option value={key} key={key}>#{item.label} ({item.count})</option>)}</select></label>{sessionFilter && <button onClick={() => setSessionFilter('')}>Quitar filtro ×</button>}</nav>
        <div className={"nb2-columns" + (mobileReader ? " is-reading" : "")}>
          <aside className="nb2-note-list" aria-label="Entradas"><header><div><small>MEMORIA DE LA MESA</small><h2>Entradas</h2></div><span>{visibleNotes.length}</span></header>{groups.map((group) => <section key={group.name}><div className="nb2-group-heading"><h3>{group.name}</h3>{overview && group.items.length > 3 && <button onClick={() => setFilter(group.filter)}>Ver todas</button>}</div>{(overview ? group.items.slice(0, 3) : group.items).map((note) => <button className="nb2-note-item" aria-pressed={activeNote?.id === note.id} key={note.id} onClick={() => { setSelectedId(note.id); setMobileReader(true) }}><strong>{note.title}</strong><span>{note.author.displayName} · {date(note.updatedAt)}</span><small>{note.visibility === 'PRIVATE' ? 'Privada' : 'Compartida'}{note.pinned ? ' · Fijada' : ''}</small><p>{readable(note.content).slice(0, 120)}</p></button>)}{!group.items.length && <p className="nb2-muted">No hay entradas en este apartado.</p>}</section>)}</aside>
          <article className="nb2-reader" aria-label="Lectura de la nota"><button className="nb2-mobile-back" type="button" onClick={() => setMobileReader(false)}>← Volver a entradas</button>{activeNote ? <><header><div><small>{activeNote.visibility === 'PRIVATE' ? 'MI NOTA · PRIVADA' : 'NOTA COMPARTIDA'}</small><h2>{activeNote.title}</h2><p>{activeNote.author.displayName} · {date(activeNote.updatedAt)}</p></div><div className="nb2-actions"><button onClick={() => void pin(activeNote)} aria-label={activeNote.pinned ? 'Desfijar entrada' : 'Fijar entrada'}>{activeNote.pinned ? '★' : '☆'}</button>{activeNote.canEdit && <button onClick={() => startNote(undefined, activeNote)}>Editar</button>}</div></header>{contextLocationImage ? <figure className="nb2-context-location-image"><img src={contextLocationImage.imageUrl} alt={contextLocationImage.name} /><figcaption>Ubicación contextual · {contextLocationImage.name}</figcaption></figure> : null}<div className="nb2-prose">{renderContent(activeNote.content)}</div>{activeNote.references.length > 0 && <div className="nb2-linked"><h3>Referencias de esta entrada</h3>{activeNote.references.map((ref) => <button key={ref.id} onClick={() => void openResource({ ...ref, label: ref.label || 'Recurso' })}>{ref.label || labels[ref.targetType] || 'Recurso'} ↗</button>)}</div>}<footer><div className="nb2-tag-cloud">{activeNote.tags.map((item) => <button key={item} onClick={() => filterTag(item)}>#{item}</button>)}{!activeNote.tags.length && <span>Sin etiquetas</span>}</div>{activeNote.canEdit && <button onClick={() => void archive(activeNote)}>Archivar entrada</button>}</footer></> : <p>No hay notas con estos filtros.</p>}</article>
          <aside className="nb2-context"><section className="nb2-preview-slot" aria-live="polite"><small>VISTA PREVIA DEL RECURSO</small>{preview ? <><button className="nb2-preview-close" type="button" aria-label="Cerrar vista previa" onClick={() => { setPreview(null); setPreviewError('') }}>×</button><span className="nb2-preview-link">ENLACE DESDE LA NOTA</span><h2>{preview.label}</h2>{preview.imageUrl && <img className="nb2-preview-image" src={preview.imageUrl} alt="" />}<p className="nb2-preview-type">{labels[preview.targetType] || preview.targetType}{preview.category ? ' · ' + preview.category : ''}</p><p className="nb2-preview-status">● {preview.status.toLowerCase() === 'active' ? 'Activo' : preview.status}</p><p>{preview.description || 'Sin descripción disponible.'}</p>{preview.narrativeRole ? <p><strong>Función:</strong> {preview.narrativeRole}</p> : null}<button className="nb2-primary" type="button" onClick={() => { setFullSheet(true); setPreviewOpen(true) }}>Abrir ficha completa</button><small className="nb2-preview-privacy">La información privada del Narrador no se muestra a los jugadores.</small></> : <><h2>Sin recurso seleccionado</h2><p>Pulsa una referencia resaltada dentro de la nota para consultar aquí su ficha resumida.</p></>}</section><section className="nb2-chronicle-context"><small>CONTEXTO DE LA CRÓNICA</small><h2>Ahora en juego</h2><div><span>Sesión actual</span><strong>{activeNote?.session?.title || 'Nota general de la crónica'}</strong></div><div><span>Crónica</span><strong>{chronicles.find((item) => item.id === chronicleId)?.name || '—'}</strong></div><div><span>Notas compartidas</span><strong>{notes.filter((item) => item.visibility !== 'PRIVATE').length}</strong></div></section></aside>
        </div></section>}
    </>}
    {previewOpen && <Modal title={fullSheet ? 'Ficha del recurso' : 'Vista previa del recurso'} close={closePreview} wide={fullSheet}>{previewError ? <p role="alert">{previewError}</p> : !preview ? <p role="status">Cargando ficha…</p> : <><small>{labels[preview.targetType] || preview.targetType} · {preview.category}</small><h3>{preview.label}</h3>{preview.imageUrl && <img className="nb2-sheet-image" src={preview.imageUrl} alt="" />}<p>{preview.status.toLowerCase() === 'active' ? 'Activo' : preview.status}</p><div className="nb2-prose">{preview.description || 'Sin descripción disponible.'}</div>{preview.narrativeRole && <p><strong>Función narrativa:</strong> {preview.narrativeRole}</p>}{fullSheet ? <><NotebookResourceDetails resource={preview} />{preview.parentLocationId && <button onClick={() => void openResource({ targetType: 'LOCATION', targetId: preview.parentLocationId!, label: 'Localización superior' }, true)}>Abrir localización superior</button>}{preview.narratorDetails && <section className="nb2-private"><h3>Información del narrador</h3><p>{preview.narratorDetails}</p></section>}<h3>Anotaciones del recurso ({resourceNotes.length})</h3>{resourceNotes.map((note) => <button className="nb2-row" key={note.id} onClick={() => showNote(note)}><strong>{note.title}</strong><span>{note.author.displayName} · {visibilityLabels[note.visibility]}</span></button>)}{!resourceNotes.length && <p>No hay anotaciones accesibles todavía.</p>}<button className="nb2-primary" onClick={() => startNote({ targetType: preview.targetType, targetId: preview.targetId, label: preview.label })}>Añadir nota sobre este recurso</button></> : <button className="nb2-primary" onClick={() => setFullSheet(true)}>Abrir ficha completa</button>}<p className="nb2-muted">Se muestra la información disponible para tu cuenta. Las notas privadas solo las ve su autor.</p></>}</Modal>}
