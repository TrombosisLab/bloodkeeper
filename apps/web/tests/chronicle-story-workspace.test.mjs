...
test('SPEC-060-F exposes editable milestone notes without changing completion semantics', () => {
  assert.match(workspace, /story-milestone__status/)
  assert.match(workspace, /story-milestone__select/)
  assert.match(workspace, /story-milestone-editor/)
  assert.match(workspace, /milestoneNoteDrafts/)
  assert.match(workspace, /note: milestoneNoteDrafts/)
  assert.match(workspace, /maxLength=\{2000\}/)
  assert.match(styles, /story-milestone-editor/)
  assert.match(styles, /grid-template-columns: repeat\(2,\s*minmax\(0,\s*1fr\)\)/)
  assert.doesNotMatch(workspace, /<button key=\{milestone\.key\}/)
})
